(function($$, $){
  ...
  $$.extend($$, {
    ...
    setLanguageSelector: function(){
     ...
     // Determine if the Slideshare Sunset banner should be shown and display it
    initSunsetBanner: function() {
      var $sunsetBanner = $('.j-sunset-banner');
      if (!$sunsetBanner.length || slideshare_object.is_homepage) {
        return;
      }
  ...
  function validateOrigin(origin) {
    var isValidOrigin = origin === ('http://' + location.host) || origin === ('https://' + location.host);
    if (!isValidOrigin) {
      var hostParts = location.host.split('.');
      var subdomain = hostParts[0];
      if (["www", "fr", "es", "pt", "de"].includes(subdomain)) {
        hostParts[0] = subdomain + '2';
        isValidOrigin = origin === ('http://' + hostParts.join('.')) || origin === ('https://' + hostParts.join('.'));
      }
    }
